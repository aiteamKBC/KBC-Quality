...
function CustomTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  return (
    <div className="rounded-lg bg-slate-900 px-3 py-2 text-xs text-white">
      ...
      {payload.map((point) => (
        <p key={point.name} className={point.name === "target" ? "text-slate-400" : "text-amber-300"}>
          {point.name === "target" ? "Target" : "Attendance"}: {point.value}%
        </p>
      ))}
    </div>
  );
...
export default function AttendanceChart({ data }: Props) {
  if (data.length === 0) {
    return <div className="py-10 text-center text-sm text-slate-400">No attendance trend available</div>;
  }
